import { addDoc, doc, onSnapshot, orderBy, query, runTransaction, serverTimestamp } from 'firebase/firestore';
import { ArrowDownToLine, ArrowUpFromLine, CircleArrowDown, CircleArrowUp, CalendarClock, Trash2 } from 'lucide-react';
...
export function FinanceiroPage() {
  ...
  const totalPagar = useMemo(() => contasPagar.filter((conta) => ['ABERTA', 'PARCIAL'].includes(conta.status)).reduce((soma, conta) => soma + Number(conta.valorAberto || 0), 0), [contasPagar]);
  const clienteDaConta = clientes.find((cliente) => cliente.id === formReceber.clienteId);

  const abrirContaReceber = () => { setFormReceber(contaReceberVazia); setTipoModal('RECEBER'); };
  const abrirContaPagar = () => { setFormPagar(contaPagarVazia); setTipoModal('PAGAR'); };
  ...
  const confirmarRecebimento = async () => { if (!contaRecebimento || !usuario) return; if (valorBaixa <= 0 || valorBaixa > contaRecebimento.valorAberto) return showToast('Informe um valor entre zero e o saldo em aberto.', 'error'); setSalvando(true); try { await runTransaction(db, async (transaction) => { const contaRef = empresaDoc(empresaId!, 'contasReceber', contaRecebimento.id); const snap = await transaction.get(contaRef); if (!snap.exists()) throw new Error('Conta não encontrada.'); const atual = snap.data() as ContaReceber; const saldo = Math.max(0, Number(atual.valorAberto) - Number(valorBaixa)); transaction.update(contaRef, { valorAberto: saldo, status: saldo === 0 ? 'RECEBIDA' : 'PARCIAL', atualizadoEm: serverTimestamp() }); const registroFinanceiroId = (atual as ContaReceber & { registroFinanceiroId?: string }).registroFinanceiroId; if (registroFinanceiroId) transaction.update(empresaDoc(empresaId!, 'registrosFinanceiros', registroFinanceiroId), { status: saldo === 0 ? 'RECEBIDO' : 'PARCIAL', valorAberto: saldo, atualizadoEm: serverTimestamp() }); transaction.set(doc(empresaCollection(empresaId!, 'recebimentos')), { contaReceberId: contaRecebimento.id, clienteNome: atual.clienteNome, valor: Number(valorBaixa), data: hojeIso(), formaPagamento, usuarioId: usuario.id, usuarioNome: usuario.nome, criadoEm: serverTimestamp() }); transaction.set(doc(empresaCollection(empresaId!, 'movimentacoesCaixa')), { tipo: 'ENTRADA', origem: 'RECEBIMENTO', origemId: contaRecebimento.id, descricao: `Recebimento — ${atual.descricao}`, clienteId: atual.clienteId || '', clienteNome: atual.clienteNome, valor: Number(valorBaixa), formaPagamento, data: hojeIso(), usuarioId: usuario.id, usuarioNome: usuario.nome, criadoEm: serverTimestamp() }); }); showToast('Recebimento registrado.'); setContaRecebimento(null); } catch (error) { showToast(error instanceof Error ? error.message : 'Não foi possível registrar o recebimento.', 'error'); } finally { setSalvando(false); } };
  const confirmarPagamento = async () => { if (!contaPagamento || !usuario) return; if (valorBaixa <= 0 || valorBaixa > contaPagamento.valorAberto) return showToast('Informe um valor entre zero e o saldo em aberto.', 'error'); setSalvando(true); try { await runTransaction(db, async (transaction) => { const contaRef = empresaDoc(empresaId!, 'contasPagar', contaPagamento.id); const snap = await transaction.get(contaRef); if (!snap.exists()) throw new Error('Conta não encontrada.'); const atual = snap.data() as ContaPagar; const saldo = Math.max(0, Number(atual.valorAberto) - Number(valorBaixa)); transaction.update(contaRef, { valorAberto: saldo, status: saldo === 0 ? 'PAGA' : 'PARCIAL', atualizadoEm: serverTimestamp() }); transaction.set(doc(empresaCollection(empresaId!, 'pagamentos')), { contaPagarId: contaPagamento.id, fornecedorNome: atual.fornecedorNome, valor: Number(valorBaixa), data: hojeIso(), formaPagamento, usuarioId: usuario.id, usuarioNome: usuario.nome, criadoEm: serverTimestamp() }); transaction.set(doc(empresaCollection(empresaId!, 'movimentacoesCaixa')), { tipo: 'SAIDA', origem: 'PAGAMENTO', origemId: contaPagamento.id, descricao: `Pagamento — ${atual.descricao}`, clienteNome: atual.fornecedorNome, valor: Number(valorBaixa), formaPagamento, data: hojeIso(), usuarioId: usuario.id, usuarioNome: usuario.nome, criadoEm: serverTimestamp() }); }); showToast('Pagamento registrado.'); setContaPagamento(null); } catch (error) { showToast(error instanceof Error ? error.message : 'Não foi possível registrar o pagamento.', 'error'); } finally { setSalvando(false); } };

  const excluirReceber = async (conta: ContaReceber) => {
    if (!podeExcluir) return showToast('Somente administradores podem excluir contas a receber.', 'error');
    if (!window.confirm(`Excluir definitivamente a conta a receber de “${conta.clienteNome}” no valor de ${moeda(conta.valorOriginal)}?

Recebimentos, movimentações de caixa e registros financeiros vinculados também serão removidos.`)) return;
    setExcluindoId(`receber-${conta.id}`);
    try {
      await excluirContaReceber(empresaId!, conta);
      showToast('Conta a receber excluída.');
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Não foi possível excluir a conta a receber.', 'error');
    } finally {
      setExcluindoId(null);
    }
  };

  const excluirPagar = async (conta: ContaPagar) => {
    if (!podeExcluir) return showToast('Somente administradores podem excluir contas a pagar.', 'error');
    if (!window.confirm(`Excluir definitivamente a conta a pagar de “${conta.fornecedorNome}” no valor de ${moeda(conta.valorOriginal)}?

Pagamentos e movimentações de caixa vinculados também serão removidos.`)) return;
    setExcluindoId(`pagar-${conta.id}`);
    try {
      await excluirContaPagar(empresaId!, conta);
      showToast('Conta a pagar excluída.');
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Não foi possível excluir a conta a pagar.', 'error');
    } finally {
      setExcluindoId(null);
    }
  };
  ...
  return <>
    <section className="page-heading"><div><p className="eyebrow">FINANCEIRO</p><h2>Contas e baixas</h2><p>Controle valores a receber, contas a pagar e pagamentos realizados.</p></div><div className="button-group"><button className="button button-secondary" onClick={abrirContaPagar}><ArrowUpFromLine size={18} />Lançar conta a pagar</button><button className="button button-primary" onClick={abrirContaReceber}><ArrowDownToLine size={18} />Lançar conta a receber</button></div></section>
    <div className="stats-grid"><article className="stat-card"><div className="stat-icon teal"><CircleArrowDown size={21} /></div><div><span>Total a receber</span><strong className="currency-stat">{moeda(totalReceber)}</strong></div></article><article className="stat-card"><div className="stat-icon rose"><CircleArrowUp size={21} /></div><div><span>Total a pagar</span><strong className="currency-stat">{moeda(totalPagar)}</strong></div></article><article className="stat-card"><div className="stat-icon amber"><CalendarClock size={21} /></div><div><span>Contas a receber abertas</span><strong>{contasReceber.filter((c) => ['ABERTA', 'PARCIAL'].includes(c.status)).length}</strong></div></article><article className="stat-card"><div className="stat-icon blue"><CalendarClock size={21} /></div><div><span>Contas a pagar abertas</span><strong>{contasPagar.filter((c) => ['ABERTA', 'PARCIAL'].includes(c.status)).length}</strong></div></article></div>
